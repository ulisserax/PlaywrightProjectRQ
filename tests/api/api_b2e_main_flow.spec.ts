...
test.describe.serial("Api B2E Main Flow", () => {
   ...
   test("POST: Create a b2e request", async ({v2Endpoints}) => {
      ...
      ENV.API_REQUEST_UID = `${JSON.parse(_response).request_uid}`;
   ...
   test("PUT: Go2network request", async ({v2Endpoints}) => {
      const res = await v2Endpoints.go2Network(ENV.BASE_URL, ENV.API_TOKEN, ENV.API_REQUEST_UID , '1', '1', '8', false, 0, '1', 0, false, '0', parseInt(ENV.API_RATECARD_ID), 'USD');
      const _response = JSON.parse(res);
   ...
   test("POST: CorProperties ", async ({v2Endpoints}) => {
      const res = await v2Endpoints.corProperties(ENV.BASE_URL, ENV.API_TOKEN, ENV.API_REQUEST_UID );
      const _response = JSON.parse(res);
      console.info(`Success: ${_response.success}`);
      await expect(_response.success).toEqual(true);
   ...
   test("GET: Property Details ", async ({v2Endpoints}) => {
      const res = await v2Endpoints.propertyDetails(ENV.BASE_URL, ENV.API_TOKEN, new_reference);
      const _response = JSON.parse(res);
      console.info(`Success: ${_response.success}`);
      await expect(_response.success).toEqual(true);
   ...
   test("POST: Stripe Token ", async ({stripe}) => {
      const res = await stripe.stripeToken(ENV.CREDIT_CARD, '11', '35', ENV.CARD_CVC, ENV.STRIPE_KEY, '33331', ENV.STRIPE_GUID, ENV.STRIPE_MUID, ENV.STRIPE_PAYMENT_TOKEN, ENV.STRIPE_SID);
      const _response = JSON.parse(res);
   ...
   test("POST: Accept Terms Of Reservation ", async ({v2Endpoints}) => {
      const res = await v2Endpoints.acceptTermsOfReservation(ENV.BASE_URL, ENV.API_TOKEN, ENV.API_REQUEST_UID, ENV.API_OPTION_ID );
      const _response = JSON.parse(res);
      console.info(`Success: ${_response.success}`);
      await expect(_response.success).toEqual(true);
   })
   ...
   test("POST: Book ", async ({v2Endpoints}) => {
      const res = await v2Endpoints.book(ENV.BASE_URL, ENV.API_TOKEN, rate_token, ENV.STRIPE_TOKEN );
      const _response = JSON.parse(res);
      console.info(`success: ${_response.success}`);
