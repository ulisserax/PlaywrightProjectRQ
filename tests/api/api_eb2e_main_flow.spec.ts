...
test.describe.serial("EB2E RQPRO API main flow", ()=>{
    ...
    test("POST: Create an EB2E RQPRO Request", async ({requestEndpoints}) => {
        ...
        const _response = await requestEndpoints.createRequest(ENV.RQPRO_BASE_URL, ENV.RQPRO_REQ_API_KEY, Number(ENV.RQPRO_EB2E_CLIENT), 'Miami, FL, USA', ENV.START_DATE, ENV.END_DATE, ENV.GUEST_FIRSTNAME, ENV.GUEST_LASTNAME, ENV.RQPRO_GUEST_EMAIL, ENV.GUEST_PHONE);
