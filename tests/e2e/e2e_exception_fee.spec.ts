...
test.describe("Test Suite Exception Fee", () => { //serial
    ...
    for(let i = 0; i < location.length; i++){
        test(`Create a new Request via api and validate the exception fee for ${location[i]}`, async({requestEndpoints, webActions,homePage, dashboard, search, requestShow, option}) =>{
            ...
            const _response = await requestEndpoints.createRequest(ENV.BASE_URL,ENV.REQUESTOR_API_KEY, Number(ENV.NT1_EXCEPTION_FEE_CLIENT_ID), location[i], ENV.START_DATE, ENV.END_DATE, ENV.GUEST_FIRSTNAME,ENV.GUEST_LASTNAME,ENV.GUEST_EMAIL, '7863652563');
