...
test.describe.serial.only("Test Suite Basic Flow ", () => {
    ...
    test("Validate basic emails", async ({webActions, homePage, configurationInstance, mailCatcher}) => {
        ...
        await mailCatcher.verifyBasicEmails(`Supplier New Service Issue`, ENV.SUPPLIER_COMPANY_EMAIL.toLocaleLowerCase(), `ALERT! - Service Issue has been submitted for reservation ${ENV.RESERVATION_ID}`, `p:has-text('Service Issue Submitted')`, `a:has-text('VIEW SERVICE ISSUES')`, `${ENV.SUPPLIER_DOMAIN}/request/show/${ENV.REQUEST_ID}?openServiceIssueTab=1`);
