...
import ENV  from '@utils/env';
 ...
 test.describe("Test Suite Basic Flow ", () => {
    ...
    test("Validate basic emails", async ({webActions, homePage, configurationInstance, mailCatcher}) => {
        ...
        await mailCatcher.verifyBasicEmails(`Supplier For Deadline Update`, ENV.SUPPLIER_COMPANY_EMAIL, `URGENT Updated Request: ${ENV.REQUESTOR_COMPANY}, ${ENV.REQUEST_ID}`, `//h4[contains(normalize-space(),'1 field(s) updated on')]/following-sibling::ul/li[contains(normalize-space(),'Departure date')]`, `a:has-text('${ENV.REQUEST_ID}')`,`${ENV.SUPPLIER_DOMAIN}/request/show/${ENV.REQUEST_ID}`);
        await mailCatcher.verifyBasicEmails1(`Requestor For Deadline And Assigned To Update`, ENV.REQUESTOR_EMAIL, `URGENT Updated Request: ${ENV.REQUESTOR_COMPANY}, ${ENV.REQUEST_ID}`, `//h4[contains(normalize-space(),'2 field(s) updated on')]//following-sibling::ul/li[contains(normalize-space(),'Departure date')]/following-sibling::li[contains(normalize-space(),'Assigned to')]`, `a:has-text('${ENV.REQUEST_ID}')` ,`${ENV.BASE_URL}/request/show/${ENV.REQUEST_ID}`);
        ...
        await mailCatcher.verifyBasicEmails(`Supplier New Service Issue`, ENV.SUPPLIER_EMAIL, `ALERT! - Service Issue has been submitted for reservation ${ENV.RESERVATION_ID}`, `p:has-text('Service Issue Submitted')`, `a:has-text('VIEW SERVICE ISSUES')`, `${ENV.SUPPLIER_DOMAIN}/request/show/${ENV.REQUEST_ID}?openServiceIssueTab=1`);
        await mailCatcher.verifyBasicEmails(`Guest New Service Issue`, guest_email, `Your service issues for ReloQuest reservation ${ENV.RESERVATION_ID}`, `p:has-text('Here is your list of services issues for ReloQuest reservation ${ENV.RESERVATION_ID}')`, `a:has-text('VIEW SERVICE ISSUES')` ,`${ENV.B2E_URL}/b2e/quests/`);
