...
test.describe.serial.only("Test Suite Basic Flow ", () => {
    test.slow();
    ...
    test("Validate basic emails", async ({webActions, homePage, configurationInstance, mailCatcher}) => {
        ...
        await mailCatcher.verifyBasicEmails1(`Requestor For Deadline And Assigned To Update`, ENV.REQUESTOR_EMAIL.toLocaleLowerCase(), `URGENT Updated Request: mail_subject_${ENV.REQUESTOR_COMPANY}, ${ENV.REQUEST_ID}`, `//h4[contains(normalize-space(),'2 field(s) updated on')]//following-sibling::ul/li[contains(normalize-space(),'Departure date')]/following-sibling::li[contains(normalize-space(),'Assigned from')]`, `a:has-text('${ENV.REQUEST_ID}')` ,`${ENV.BASE_URL}/request/show/${ENV.REQUEST_ID}`);
