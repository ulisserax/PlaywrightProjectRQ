...
test.describe('Share option with fees - No Guest Pay',  ()=>{
    ...
    test("SM-T1133: Requestor shares an option with fees, Client's default permissions are set to can set preferences only", async({requestEndpoints, optionEndpoints,webActions, homePage, dashboard, search, shareOption, b2eHomePage,b2eOptionsPage})=>{
        ...
        await Database.execute('Set guest_can_award=off and guest_can_select_preferences=on in the client',client_query_2);
    ...
    test("SM-T1134: Requestor shares an option with fees, Client's default permissions are set to can award only", async({requestEndpoints, optionEndpoints,webActions, homePage, dashboard, search, shareOption, b2eHomePage, b2eOptionsPage})=>{
    ...
    test("SM-T1135: Requestor shares an option with fees, Client's default permissions are set to can set preferences and can award", async({requestEndpoints, optionEndpoints,webActions, homePage, dashboard, search, shareOption, b2eHomePage, b2eOptionsPage, b2eBookingPage})=>{
    ...
    test("SM-T1140: Requestor shares an option with fees, Requestor sets sharing permissions to can set preferences only", async({requestEndpoints, optionEndpoints,webActions, homePage, dashboard, search, shareOption, b2eHomePage, b2eOptionsPage})=>{

        let data_object     = JSON.parse(ENV.NT4_PREFERENCE_DATA_OBJECT); 
        let company_query   = `UPDATE smart_company SET enable_management_services = 1, enable_eb2e = 1, enable_guest_share_version_2 = 1, enabled_guest_pay = 0, guest_pay_collect_cc = 0 , can_view_advanced_permissions_when_sharing = "ROLE_REQUESTOR" WHERE id  = ${data_object.req_company_id}`;
        let client_query_1  = `UPDATE smart_client SET enable_eb2e = 1 ,rq_pro = 1 WHERE id  = ${data_object.client_id}`;
        let client_query_2  = `UPDATE smart_inline_permission_set perm INNER JOIN smart_inline_permission_template permTmpl ON perm.id = permTmpl.permission_set_id AND permTmpl.name = 'share_profile_guest' INNER JOIN smart_client_inline_permission_template clientPermTmplRel ON permTmpl.id = clientPermTmplRel.inline_permission_template_id INNER JOIN smart_client c ON c.id = clientPermTmplRel.client_id AND c.id = ${data_object.client_id} SET perm.data = JSON_SET(perm.data, '$.EXTENDED_PERMISSIONS_AWARD_OPTION.granted',false, '$.EXTENDED_PERMISSIONS_SELECT_OPTION_PREFERENCES.granted',true )`;
    ...
    test("SM-T1141: Requestor shares an option with fees, Requestor sets sharing permissions to can award only", async({requestEndpoints, optionEndpoints,webActions, homePage, dashboard, search, shareOption, b2eHomePage, b2eOptionsPage})=>{
    ...
    test("SM-T1142: Requestor shares an option with fees, Requestor sets sharing permissions to can set preferences and can award", async({requestEndpoints, optionEndpoints,webActions, homePage, dashboard, search, shareOption, b2eHomePage, b2eOptionsPage, b2eBookingPage})=>{
